...
impl MemoryCostStore {
    pub fn new() -> Self {
        MemoryCostStore { records: Mutex::new(Vec::new()), max_records: 10_000 }
    }
}
...
impl CostStore for MemoryCostStore {
    ...
    fn daily_breakdown(&self, universe: UniverseTier, _days: u32) -> Vec<DayBucket> {
        ...
        day_map.into_iter().map(|(d, (t, c))| DayBucket { date: d, tokens: t, cost_micros: c }).collect()
    }
}
...
pub fn estimate_cost(provider: &str, prompt_tokens: u32, completion_tokens: u32) -> u64 {
    let price_per_1k: u64 = match provider {
        "spectrum" | "Spectrum" => {
            std::env::var("REACHLOCK_SPECTRUM_PRICE_PER_1K")
...
impl ProviderHealth {
    pub fn new() -> Self {
        ProviderHealth { results: VecDeque::with_capacity(100), max_samples: 100 }
    }
    pub fn record(&mut self, success: bool) {
        self.results.push_back(success);
        while self.results.len() > self.max_samples { self.results.pop_front(); }
    }
    pub fn is_tripped(&self) -> bool {
        let recent: Vec<&bool> = self.results.iter().rev().take(50).collect();
        if recent.len() < 10 { return false; }
...
impl Default for ProviderHealth { fn default() -> Self { Self::new() } }
