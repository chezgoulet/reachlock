...
impl Editor for HullMeshEditor {
    ...
    fn ui(&mut self, ui: &mut egui::Ui) {
        egui::TopBottomPanel::top("hull_mesh_toolbar").show_inside(ui, |ui| {
            ui.horizontal(|ui| {
    ...
    fn save_all(&mut self) -> Result<bool, String> {
        ...
        for entry in &mut self.entries {
            ...
            let Some(path) = &entry.path else {
                let dir = content_root().join(ContentType::HullMesh.directory());
                let _ = std::fs::create_dir_all(&dir);
                let p = crate::io::new_entry_path(&dir, &entry.file.id, &format!("hull_{wrote}"));
