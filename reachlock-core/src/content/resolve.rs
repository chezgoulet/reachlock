...
pub enum Resolved {
    /// An authored file wins; render it instead of generating. Boxed because
    /// `ContentFile` embeds full meshes — keeps the common `Procedural`
    /// variant cheap (clippy `large_enum_variant`).
    Authored(Box<ContentFile>),
...
pub fn resolve(overrides: &[ContentFile], params: &SeedParams) -> Resolved {
    overrides
        ...
        .cloned()
        .map(|c| Resolved::Authored(Box::new(c)))
