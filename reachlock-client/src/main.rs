...
fn main() {
    ...
    let mut app = App::new();
    if let Some(request) = screenshot::ScreenshotRequest::from_env() {
    ...
    app.add_plugins(
        DefaultPlugins
            ...
            // Assets resolve from the install root, not the working
            // directory, so fonts and the stylesheet load the same way
            // whether the game is run from the repo, from `make run`, or
            // from an installed copy (see `reachlock_core::paths`).
            .set(AssetPlugin {
                file_path: reachlock_core::paths::install_root()
                    .join("assets")
                    .to_string_lossy()
                    .into_owned(),
                ..default()
    ...
    // The stylesheet layer. Must be added before anything spawns UI.
    .add_plugins(theme::ThemePlugin)
    // Env-gated screenshot capture (REACHLOCK_SCREENSHOT=path). Absent
    // variable means the resource never exists and the system no-ops.
    .add_systems(Update, screenshot::capture_screenshot)
    ...
    // The menu is spawned on every entry to MainMenu and torn down on
    // exit. Spawning it once at startup instead is what left a dead grey
    // screen behind when the player backed out of character creation.
    .add_systems(OnEnter(AppState::MainMenu), menu::spawn_menu)
    .add_systems(OnExit(AppState::MainMenu), menu::despawn_menu)
    .add_systems(
        Update,
        (
            menu::menu_input,
            menu::update_menu_selection,
    ...
    // S34: contract library browser — browse, sort, filter, import
    // contracts. Keyboard-driven, accessible from a crew console.
    .add_systems(
        Update,
        (
            contract_library::library_system,
            contract_library::render_library_panel,
        )
            .chain()
            .run_if(in_any_interior),
    )
    ...
    .add_systems(
        Update,
        (
            deliberation_renderer::advance_deliberation_stage.run_if(in_state(AppState::InGame)),
